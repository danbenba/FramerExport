const FRAGMENT_LINES = [
  '#version 300 es',
  'precision highp float;',
  'uniform vec3  uColor;',
  'uniform vec2  uResolution;',
  'uniform float uTime;',
  'uniform float uPixelSize;',
  'uniform float uScale;',
  'uniform float uDensity;',
  'uniform float uPixelJitter;',
  'uniform int   uEnableRipples;',
  'uniform float uRippleSpeed;',
  'uniform float uRippleThickness;',
  'uniform float uRippleIntensity;',
  'uniform float uEdgeFade;',
  'uniform int   uShapeType;',
  'const int SHAPE_SQUARE   = 0;',
  'const int SHAPE_CIRCLE   = 1;',
  'const int SHAPE_TRIANGLE = 2;',
  'const int SHAPE_DIAMOND  = 3;',
  'const int MAX_CLICKS = 10;',
  'uniform vec2  uClickPos  [MAX_CLICKS];',
  'uniform float uClickTimes[MAX_CLICKS];',
  'out vec4 fragColor;',
  'float Bayer2(vec2 a) { a = floor(a); return fract(a.x / 2. + a.y * a.y * .75); }',
  '#define Bayer4(a) (Bayer2(.5*(a))*0.25 + Bayer2(a))',
  '#define Bayer8(a) (Bayer4(.5*(a))*0.25 + Bayer2(a))',
  '#define FBM_OCTAVES     5',
  '#define FBM_LACUNARITY  1.25',
  '#define FBM_GAIN        1.0',
  'float hash11(float n){ return fract(sin(n)*43758.5453); }',
  'float vnoise(vec3 p){',
  '  vec3 ip = floor(p);',
  '  vec3 fp = fract(p);',
  '  float n000 = hash11(dot(ip + vec3(0.0,0.0,0.0), vec3(1.0,57.0,113.0)));',
  '  float n100 = hash11(dot(ip + vec3(1.0,0.0,0.0), vec3(1.0,57.0,113.0)));',
  '  float n010 = hash11(dot(ip + vec3(0.0,1.0,0.0), vec3(1.0,57.0,113.0)));',
  '  float n110 = hash11(dot(ip + vec3(1.0,1.0,0.0), vec3(1.0,57.0,113.0)));',
  '  float n001 = hash11(dot(ip + vec3(0.0,0.0,1.0), vec3(1.0,57.0,113.0)));',
  '  float n101 = hash11(dot(ip + vec3(1.0,0.0,1.0), vec3(1.0,57.0,113.0)));',
  '  float n011 = hash11(dot(ip + vec3(0.0,1.0,1.0), vec3(1.0,57.0,113.0)));',
  '  float n111 = hash11(dot(ip + vec3(1.0,1.0,1.0), vec3(1.0,57.0,113.0)));',
  '  vec3 w = fp*fp*fp*(fp*(fp*6.0-15.0)+10.0);',
  '  float x00 = mix(n000, n100, w.x);',
  '  float x10 = mix(n010, n110, w.x);',
  '  float x01 = mix(n001, n101, w.x);',
  '  float x11 = mix(n011, n111, w.x);',
  '  float y0  = mix(x00, x10, w.y);',
  '  float y1  = mix(x01, x11, w.y);',
  '  return mix(y0, y1, w.z) * 2.0 - 1.0;',
  '}',
  'float fbm2(vec2 uv, float t){',
  '  vec3 p = vec3(uv * uScale, t);',
  '  float amp = 1.0;',
  '  float freq = 1.0;',
  '  float sum = 1.0;',
  '  for (int i = 0; i < FBM_OCTAVES; ++i){',
  '    sum  += amp * vnoise(p * freq);',
  '    freq *= FBM_LACUNARITY;',
  '    amp  *= FBM_GAIN;',
  '  }',
  '  return sum * 0.5 + 0.5;',
  '}',
  'float maskCircle(vec2 p, float cov){',
  '  float r = sqrt(cov) * .25;',
  '  float d = length(p - 0.5) - r;',
  '  float aa = 0.5 * fwidth(d);',
  '  return cov * (1.0 - smoothstep(-aa, aa, d * 2.0));',
  '}',
  'float maskTriangle(vec2 p, vec2 id, float cov){',
  '  bool flip = mod(id.x + id.y, 2.0) > 0.5;',
  '  if (flip) p.x = 1.0 - p.x;',
  '  float r = sqrt(cov);',
  '  float d  = p.y - r*(1.0 - p.x);',
  '  float aa = fwidth(d);',
  '  return cov * clamp(0.5 - d/aa, 0.0, 1.0);',
  '}',
  'float maskDiamond(vec2 p, float cov){',
  '  float r = sqrt(cov) * 0.564;',
  '  return step(abs(p.x - 0.49) + abs(p.y - 0.49), r);',
  '}',
  'void main(){',
  '  float pixelSize = uPixelSize;',
  '  vec2 fragCoord = gl_FragCoord.xy - uResolution * .5;',
  '  float aspectRatio = uResolution.x / uResolution.y;',
  '  vec2 pixelId = floor(fragCoord / pixelSize);',
  '  vec2 pixelUV = fract(fragCoord / pixelSize);',
  '  float cellPixelSize = 8.0 * pixelSize;',
  '  vec2 cellId = floor(fragCoord / cellPixelSize);',
  '  vec2 cellCoord = cellId * cellPixelSize;',
  '  vec2 uv = cellCoord / uResolution * vec2(aspectRatio, 1.0);',
  '  float base = fbm2(uv, uTime * 0.05);',
  '  base = base * 0.5 - 0.65;',
  '  float feed = base + (uDensity - 0.5) * 0.3;',
  '  float speed     = uRippleSpeed;',
  '  float thickness = uRippleThickness;',
  '  const float dampT     = 1.0;',
  '  const float dampR     = 10.0;',
  '  if (uEnableRipples == 1) {',
  '    for (int i = 0; i < MAX_CLICKS; ++i){',
  '      vec2 pos = uClickPos[i];',
  '      if (pos.x < 0.0) continue;',
  '      vec2 cuv = (((pos - uResolution * .5 - cellPixelSize * .5) / (uResolution))) * vec2(aspectRatio, 1.0);',
  '      float t = max(uTime - uClickTimes[i], 0.0);',
  '      float r = distance(uv, cuv);',
  '      float waveR = speed * t;',
  '      float ring  = exp(-pow((r - waveR) / thickness, 2.0));',
  '      float atten = exp(-dampT * t) * exp(-dampR * r);',
  '      feed = max(feed, ring * atten * uRippleIntensity);',
  '    }',
  '  }',
  '  float bayer = Bayer8(fragCoord / uPixelSize) - 0.5;',
  '  float bw = step(0.5, feed + bayer);',
  '  float h = fract(sin(dot(floor(fragCoord / uPixelSize), vec2(127.1, 311.7))) * 43758.5453);',
  '  float jitterScale = 1.0 + (h - 0.5) * uPixelJitter;',
  '  float coverage = bw * jitterScale;',
  '  float M;',
  '  if      (uShapeType == SHAPE_CIRCLE)   M = maskCircle (pixelUV, coverage);',
  '  else if (uShapeType == SHAPE_TRIANGLE) M = maskTriangle(pixelUV, pixelId, coverage);',
  '  else if (uShapeType == SHAPE_DIAMOND)  M = maskDiamond(pixelUV, coverage);',
  '  else                                   M = coverage;',
  '  if (uEdgeFade > 0.0) {',
  '    vec2 norm = gl_FragCoord.xy / uResolution;',
  '    float edge = min(min(norm.x, norm.y), min(1.0 - norm.x, 1.0 - norm.y));',
  '    float fade = smoothstep(0.0, uEdgeFade, edge);',
  '    M *= fade;',
  '  }',
  '  vec3 color = uColor;',
  '  vec3 srgbColor = mix(',
  '    color * 12.92,',
  '    1.055 * pow(color, vec3(1.0 / 2.4)) - 0.055,',
  '    step(0.0031308, color)',
  '  );',
  '  fragColor = vec4(srgbColor, M);',
  '}',
];

const VERTEX_LINES = [
  '#version 300 es',
  'in vec2 position;',
  'void main() {',
  '  gl_Position = vec4(position, 0.0, 1.0);',
  '}',
];

const CLIENT_LINES = [
  '(function () {',
  '  var SHAPES = { square: 0, circle: 1, triangle: 2, diamond: 3 };',
  '  var MAX_CLICKS = 10;',
  '  function hexToRgb(hex) {',
  "    var value = hex.replace('#', '');",
  '    var n = parseInt(value, 16);',
  '    return [((n >> 16) & 255) / 255, ((n >> 8) & 255) / 255, (n & 255) / 255];',
  '  }',
  '  function compile(gl, type, source) {',
  '    var shader = gl.createShader(type);',
  '    gl.shaderSource(shader, source);',
  '    gl.compileShader(shader);',
  '    if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {',
  '      throw new Error(gl.getShaderInfoLog(shader) || "shader compile failed");',
  '    }',
  '    return shader;',
  '  }',
  '  function mount(canvas, opts) {',
  '    opts = opts || {};',
  '    var gl = canvas.getContext("webgl2", { alpha: true, antialias: false });',
  '    if (!gl) { canvas.dataset.renderer = "unavailable"; return null; }',
  '    var program = gl.createProgram();',
  '    gl.attachShader(program, compile(gl, gl.VERTEX_SHADER, window.__PB_VERT__));',
  '    gl.attachShader(program, compile(gl, gl.FRAGMENT_SHADER, window.__PB_FRAG__));',
  '    gl.linkProgram(program);',
  '    if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {',
  '      throw new Error(gl.getProgramInfoLog(program) || "link failed");',
  '    }',
  '    gl.useProgram(program);',
  '    var buffer = gl.createBuffer();',
  '    gl.bindBuffer(gl.ARRAY_BUFFER, buffer);',
  '    gl.bufferData(gl.ARRAY_BUFFER, new Float32Array([-1, -1, 3, -1, -1, 3]), gl.STATIC_DRAW);',
  '    var loc = gl.getAttribLocation(program, "position");',
  '    gl.enableVertexAttribArray(loc);',
  '    gl.vertexAttribPointer(loc, 2, gl.FLOAT, false, 0, 0);',
  '    gl.enable(gl.BLEND);',
  '    gl.blendFunc(gl.SRC_ALPHA, gl.ONE_MINUS_SRC_ALPHA);',
  '    var U = {};',
  '    ["uColor","uResolution","uTime","uPixelSize","uScale","uDensity","uPixelJitter","uEnableRipples","uRippleSpeed","uRippleThickness","uRippleIntensity","uEdgeFade","uShapeType","uClickPos","uClickTimes"].forEach(function (name) {',
  '      U[name] = gl.getUniformLocation(program, name);',
  '    });',
  '    canvas.dataset.renderer = "webgl2";',
  '    var color = hexToRgb(opts.color || "#FAB283");',
  '    var pixelSize = opts.pixelSize || 5;',
  '    var dpr = Math.min(window.devicePixelRatio || 1, 2);',
  '    var clicks = new Float32Array(MAX_CLICKS * 2).fill(-1);',
  '    var clickTimes = new Float32Array(MAX_CLICKS);',
  '    var clickIx = 0;',
  '    var timeOffset = 240;',
  '    var speed = opts.speed || 0.5;',
  '    var start = performance.now();',
  '    var raf = 0, disposed = false, visible = true, suspended = false, lastFrame = 0;',
  '    var motion = window.matchMedia("(prefers-reduced-motion: reduce)");',
  '    function paused() { return disposed || suspended || !visible || document.hidden || motion.matches || document.documentElement.dataset.reduceMotion !== "false"; }',
  '    function update() {',
  '      cancelAnimationFrame(raf); raf = 0;',
  '      canvas.dataset.motion = paused() ? "paused" : "running";',
  '      if (!disposed && !suspended && visible && !document.hidden) { draw(performance.now()); if (!paused()) raf = requestAnimationFrame(frame); }',
  '    }',
  '    function resize() {',
  '      if (disposed) return;',
  '      dpr = Math.min(window.devicePixelRatio || 1, 1.5);',
  '      var w = canvas.clientWidth || window.innerWidth;',
  '      var h = canvas.clientHeight || window.innerHeight;',
  '      canvas.width = Math.floor(w * dpr);',
  '      canvas.height = Math.floor(h * dpr);',
  '      gl.viewport(0, 0, canvas.width, canvas.height);',
  '      update();',
  '    }',
  '    window.addEventListener("resize", resize);',
  '    function ripple(e) {',
  '      if (paused()) return;',
  '      var rect = canvas.getBoundingClientRect();',
  '      var fx = (e.clientX - rect.left) * (canvas.width / rect.width);',
  '      var fy = (rect.height - (e.clientY - rect.top)) * (canvas.height / rect.height);',
  '      clicks[clickIx * 2] = fx;',
  '      clicks[clickIx * 2 + 1] = fy;',
  '      clickTimes[clickIx] = timeOffset + ((performance.now() - start) / 1000) * speed;',
  '      clickIx = (clickIx + 1) % MAX_CLICKS;',
  '    }',
  '    document.addEventListener("pointerdown", ripple, { passive: true });',
  '    function draw(now) {',
  '      var t = timeOffset + (paused() ? 0 : ((now - start) / 1000) * speed);',
  '      gl.uniform3f(U.uColor, color[0], color[1], color[2]);',
  '      gl.uniform2f(U.uResolution, canvas.width, canvas.height);',
  '      gl.uniform1f(U.uTime, t);',
  '      gl.uniform1f(U.uPixelSize, pixelSize * dpr);',
  '      gl.uniform1f(U.uScale, opts.patternScale || 3);',
  '      gl.uniform1f(U.uDensity, opts.patternDensity || 1.2);',
  '      gl.uniform1f(U.uPixelJitter, opts.pixelSizeJitter || 0.5);',
  '      gl.uniform1i(U.uEnableRipples, paused() || opts.enableRipples === false ? 0 : 1);',
  '      gl.uniform1f(U.uRippleSpeed, opts.rippleSpeed || 0.4);',
  '      gl.uniform1f(U.uRippleThickness, opts.rippleThickness || 0.12);',
  '      gl.uniform1f(U.uRippleIntensity, opts.rippleIntensityScale || 1.5);',
  '      gl.uniform1f(U.uEdgeFade, opts.edgeFade == null ? 0.25 : opts.edgeFade);',
  '      gl.uniform1i(U.uShapeType, SHAPES[opts.variant || "circle"] || 0);',
  '      gl.uniform2fv(U.uClickPos, clicks);',
  '      gl.uniform1fv(U.uClickTimes, clickTimes);',
  '      gl.clearColor(0, 0, 0, 0);',
  '      gl.clear(gl.COLOR_BUFFER_BIT);',
  '      gl.drawArrays(gl.TRIANGLES, 0, 3);',
  '    }',
  '    function frame(now) {',
  '      raf = 0; if (paused()) { canvas.dataset.motion = "paused"; return; }',
  '      if (now - lastFrame >= 40) { draw(now); lastFrame = now; }',
  '      raf = requestAnimationFrame(frame);',
  '    }',
  '    var observer = new MutationObserver(update);',
  '    observer.observe(document.documentElement, { attributes: true, attributeFilter: ["data-reduce-motion"] });',
  '    var intersection = new IntersectionObserver(function(entries) { visible = entries[0].isIntersecting; update(); });',
  '    intersection.observe(canvas);',
  '    document.addEventListener("visibilitychange", update);',
  '    motion.addEventListener("change", update);',
  '    function pageHide(event) { if (!event.persisted) { dispose(); return; } suspended = true; update(); }',
  '    function pageShow() { suspended = false; update(); }',
  '    window.addEventListener("pagehide", pageHide);',
  '    window.addEventListener("pageshow", pageShow);',
  '    function dispose() {',
  '      if (disposed) return; disposed = true; cancelAnimationFrame(raf);',
  '      observer.disconnect(); intersection.disconnect();',
  '      window.removeEventListener("resize", resize); document.removeEventListener("pointerdown", ripple);',
  '      document.removeEventListener("visibilitychange", update); motion.removeEventListener("change", update);',
  '      window.removeEventListener("pagehide", pageHide); window.removeEventListener("pageshow", pageShow);',
  '      (gl.getAttachedShaders(program) || []).forEach(function(shader) { gl.detachShader(program, shader); gl.deleteShader(shader); });',
  '      gl.deleteBuffer(buffer); gl.deleteProgram(program); canvas.dataset.motion = "disposed";',
  '    }',
  '    canvas.addEventListener("webglcontextlost", function(event) { event.preventDefault(); dispose(); canvas.dataset.renderer = "unavailable"; }, { once: true });',
  '    resize();',
  '    return { gl: gl, dispose: dispose };',
  '  }',
  '  window.PixelBlast = { mount: mount };',
  '})();',
];

export const PIXEL_BLAST_JS = [
  'window.__PB_VERT__ = ' + JSON.stringify(VERTEX_LINES.join('\n')) + ';',
  'window.__PB_FRAG__ = ' + JSON.stringify(FRAGMENT_LINES.join('\n')) + ';',
  CLIENT_LINES.join('\n'),
].join('\n');

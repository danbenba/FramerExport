...
export async function runAiPromptAssistant(exporter: ExporterContext): Promise<void> {
  ...
  if (copyChoice === 'yes') {
    try {
      ...
      console.log(`  ${chalk.green('✓')} ${chalk.white.bold('Prompt copied to clipboard')}\n`);
    } catch (error) {
      console.log(
        `  ${chalk.yellow('!')} ${chalk.yellow('Clipboard copy unavailable:')} ${chalk.gray((error as Error).message)}\n`
      );
    }
  }
}
...
function printPromptResult(
  ...
  if (!isSmall) {
    ...
    console.log(boxLine(w, chalk.bold.white('  AI Prompt Ready - BETA')));
    ...
    console.log(chalk.bold.white('  AI Prompt Ready - BETA'));
    console.log(`  Tool: ${chalk.hex('#D4A017')(aiTool.displayName)}`);
    console.log(`  Stack: ${chalk.hex('#D4A017')(target.label)}`);
    console.log(`  Situation: ${chalk.hex('#D4A017')(goal.label)}`);
    console.log(`  File: ${chalk.hex('#D4A017')(relPath)}`);
  }
  console.log('');
}
